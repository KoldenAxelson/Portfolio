...
import { attrs, centredLabel, cycleForever, figureCanvas, reduceMotion, svg, whileMounted } from './figure-kit';
import { renderPythonFigure } from './def-figures-python';
...
export interface Figure {
  ...
  levels?: number;
  label?: string;
  /** Python for ML figures (def-figures-python.ts). */
  axis?: number;
  op?: string;
  keepdims?: boolean;
...
export function renderFigure(figure: Figure): HTMLElement | null {
  switch (figure.kind) {
    ...
    default:
      return renderPythonFigure(figure);
  }
}

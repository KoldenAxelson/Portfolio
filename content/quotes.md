...
<!--
  ADDING A QUOTE
  Every list item below is one quote — add a line, save, done.
  No template or front-matter edits needed.

  · Order doesn't matter; the page shuffles client-side.
  · Surrounding quote marks are optional — the layout strips them.
  · Inline Markdown works (*emphasis*, `code`, [links](/)) — though the dot
    field draws plain text, so it only shows in the plain-text fallback list.
  · Anything that isn't a list item (headings, prose, this comment) is
    ignored by the parser in layouts/_default/quotes.html, so feel free to
    leave yourself notes down here.
-->
